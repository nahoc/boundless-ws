...
const PROOF_MARKET_CONTRACT_ADDRESS = "0x01e4130C977b39aaa28A744b8D3dEB23a5297654";
...
function getCustomerAddr(id: string) {
  // Shift right by 32 bits to remove the 32-bit index
  const addrBigInt = BigInt(id) >> 32n;

  return `0x${addrBigInt.toString(16).padStart(40, "0").toLowerCase()}` as Address;
}
...
export class OrderStreamClient {
  ...
  private async processNextBatch() {
    ...
    if (this.orderQueue.length >= this.MAX_QUEUE_SIZE) {
      console.warn(`Order queue size (${this.orderQueue.length}) has reached limit. Dropping older orders.`);
      ...
      const values = batch
        .map((orderData) => {
          ...
          const typedMessage = {
            ...
            requirements: {
              imageId: request.requirements.imageId as Address,
              predicate: {
                predicateType: Number(request.requirements.predicate.predicateType === "PrefixMatch" ? 0 : 1),
                data: request.requirements.predicate.data as Address,
              },
            },
            input: {
              inputType: Number(request.input.inputType === "Inline" ? 0 : 1),
              data: request.input.data as Address,
            },
            offer: {
            ...
            imageUrl: request.imageUrl as string,
          };
          ...
          const order = {
            ...
            predicate_type: request.requirements.predicate.predicateType === "PrefixMatch" ? 0n : 1n,
  ...
  private async establishConnection() {
    const nonceResponse = await fetch(`${this.baseUrl}/api/nonce/${this.address}`);
